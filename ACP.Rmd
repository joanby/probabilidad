---
title: "Reducción de la factorialidad. Componentes pricipales."
...
También lo denotaremos simplemente $\boldsymbol\mu$.
...
posee  una medida de su dispersión respecto de su valor esperado $\boldsymbol\mu$.


Es su llamada \red{matriz de covarianzas}  y se define como
...
(X_1-\mu_1, X_2-\mu_2,\ldots,X_p-\mu_p)\right)\\ &=%
...
La matriz de covarianzas de $\underline{X}$ también se suele representar por $\mathbf{\Sigma}$.
...
es semi definida positiva  si para  todo
$\begin{pmatrix} y_{1} \\
y_{2}\\
\vdots\\
y_{n} 
...
Si la desigualdad es estricta se dice que la matriz es \red{definida positiva},
...
\blue{Teorema}
{\it La matriz de correlaciones de un vector aleatorio $\underline{X}$ es igual a la matriz de covarianzas de su vector tipificado $\underline{Z}$:}
$$
...
$$
Cov(Z_i,Z_j)=Cor(Z_i,Z_j)=Cor\left(\frac{1}{\sigma_i}X_i-\frac{\mu_i}{\sigma_i},\frac{1}{\sigma_j}X_j-\frac{\mu_j}{\sigma_j},\right)=Cor(X_i,X_j)
$$
...
Estas observaciones se pueden organizar de manera matricial, de manera que cada \red{fila} sea una realización de $\underline{X}$:
$$
...
Tanto ${s}^2_{\mathbf{X}}$ como $\widetilde{s}^2_{\mathbf{X}}$ son estimadores del vector de varianzas de $\underline{X}$: el primero es el máximo verosímil y el segundo  es insesgado. Y tanto ${s}_{\mathbf{X}}$ como $\widetilde{s}_{\mathbf{X}}$ son estimadores del vector de desviaciones típicas de $\underline{X}$: de nuevo, el primero es el máximo verosímil y el segundo  es insesgado.
...
\begin{eqnarray*}
`data frame`rac{1}{n} \mathbf{1}_n\cdot \mathbf{X} &=& \frac{1}{n}(1,\ldots,1)\cdot
...
\overline{\mathbf{X}}=`data frame`rac{1}{n} \mathbf{1}_n\cdot \mathbf{X}
$$
concluimos que
$$
\widetilde{\mathbf{X}}=\mathbf{X}-`data frame`rac{1}{n}\mathbf{1}_n^t \cdot \mathbf{1}_n\cdot \mathbf{X}
=(I_n-`data frame`rac{1}{n}\mathbf{1}_n^t \cdot \mathbf{1}_n)\mathbf{X}
$$
Si definimos
$$
\mathbf{H}_n=I_n-\frac{1}{n} \mathbf{1}_n^t\cdot \mathbf{1}_n.
$$
## Centralización de una matriz de datos; cálculo matricialel  propiedades
...
\red{Observación}. \red{Si hubiéramos partido de la matriz de datos $\textbf{X}$ dada como un `data frame`, lo que será muy común en la práctica, lo más sencillo es primero convertir el `data frame`\ en una matriz aplicándole \texttt{as.matrix} (y cambiándole el nombre, para no reescribir el `data frame`) y luego operar con esta matriz.}
...
## Matriz de covarianz<as muestrales
...
## Ejemplo  cálculo de la matriz de covarianz<as muestrales
...
Queremos recalcar que, con en el caso de la varianza con `var`, R  calcula con `cov` la versión muestral de la covarianza. Para pasar de la covarianza muestral a la covarianza, hay que multiplicar por $(n-1)/n$.
...
los datos de dicha matriz. Es una matriz simétrica y semidefinida positiva (tiene todos sus valores propios  $\geq 0$). Además, se tiene el resultado siguiente, que permite su cálculo matricial.

 \blue{Teorema}
{\it Si $\mathbf{S}$ es la matriz de covarianzas de $\mathbf{X}$, entonces}
$$
...
## Matriz de varianzas covarianzas; ejemplo
 ...
 \blue{Ejemplo}
Continuemos con el Ejemplo R , donde, recordemos,
$$
\mathbf{X}=\begin{pmatrix}
1&-1&3\\
1&0&3\\
...
\displaystyle 
s_{1 2}=\frac{1}{4}\sum_{i=1}^n x_{i 1} x_{i 2} -\overline{\mathbf{x}}_1
\overline{\mathbf{x}}_2=  \frac{5}{4}-\frac{7}{4}\cdot \frac{2}{4}=\frac{3}{8}\\[2ex]
\displaystyle 
s_{1 3}=\frac{1}{4}\sum_{i=1}^n x_{i 1} x_{i 3} -\overline{\mathbf{x}}_1
\overline{\mathbf{x}}_3=  \frac{9}{4}-\frac{7}{4}\cdot  \frac{7}{4}=-\frac{13}{16}\\[2ex]
\displaystyle 
s_{2 3}=\frac{1}{4}\sum_{i=1}^n x_{i 2} x_{i 3} -\overline{\mathbf{x}}_2
...
\begin{eqnarray*}
\mathbf{S}&=&`data frame`rac{1}{4} \mathbf{X}^t\cdot \mathbf{H}_4\cdot \mathbf{X}\\
& = & `data frame`rac{1}{4}\left(
\begin{array}{cccc}
 1 & 1 & 2 & 3 \\
 -1 & 0 & 3 & 0 \\
 3 & 3 & 0 & 1
...
```{r}
X=cbind(c(1,1,2,3),c(-1,0,3,0),c(3,3,0,1))
cov(X)  #matriz de covarianzas muestrales(3/4)*cov(X)  #matriz de covarianzas
```
...
\end{array}\right)
$$
$\mathbf{x}_{\bullet  3}$ es redundante, puesto que
$$
\left(\begin{array}{c}
2\\
7\\
...
Ahora bien, por un lado,  $|\mathbf{S}|=0$ implica que el rango de $\mathbf{S}$ es $\leq 2$, y por otro,
está claro que el rango de $\mathbf{S}$ es como mínimo 2, puesto que, por ejemplo, las dos primeras columnas son linealmente independientes (en la última fila, la primera columna tiene un 0 y la segunda, una entrada $\neq 0$). Por lo tanto, el rango de $\mathbf{S}$ es 2 y $\mathbf{X}$ contiene una  única variable redundante.
...
(el último, `3.238150e-17`). Cada uno de estos dos resultados (equivalentes) implica que hay una variable redundante.
...
es la raíz cuadrada positiva de su varianza generalizada. Se puede demostrar que ésta última es el (hiper)volumen del (hiper)poliedro de $\mathbb{R}^p$ definido por las filas de 
$\mathbf{X}$ consideradas como puntos de $\mathbb{R}^p$.
...
La matriz de correlaciones de una matriz de datos se puede calcular con R con la misma instrucción `\texttt{cor}.`cor`.
...
También podemos comprobar que esta matriz de correlaciones es la matriz de covarianzas de su matriz de datos tipificados. Recordemos que la matriz de datos tipificados  la hemos calculado en el Ejemplo  anterior
