...
Veamos cómo podemos demostralo
...
Efectivamente recordemos que la variaza de una suma (que siempre es positiva ) es  igual  una suma ponderada de todas las covarianzas, efectivamente 
...
&=&\sum_{i=1}^p\sum_{j=1}^p a_i\cdot a_j \cdot
 Cov(X_i,X_j)=  Var\left(\sum_{i=1}^p a_i\cdot  X_i\right)\geq 0
\end{eqnarray*}
...
a_{j-1} \cdot \mathbf{x}_{\bullet j-11}+a_{j+1}\cdot  +\mathbf{x}_{\bullet j+1}
