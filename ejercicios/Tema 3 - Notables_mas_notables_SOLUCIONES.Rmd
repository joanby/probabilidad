...
Y por último $Var(X)=E(X^2)-(E(X))^2=`r sumx2`-(`r mu`)^2=`r sigma2`$ y  la desviación tçípica es $\sqrt{Var(X)}=`r sqrt(sigma2)`.$
...
Efectivamente  Es sufciente sustituir en la fórmula original.

c)  Buscad un paquete de R que implemente la distribución Gumbel. Aseguraros  de que es  la (Gumbel Tipo I). Dejando fijo el parámetro $\beta=1$ dibujar la densidad Gumbel para varios valores de $\mu$ y explicad en que afecta a la gráfica el cambio de $\mu$.

Un paquete que implementa la gumbel es `extraDistr` el parámetro `mu` es $\mu$ mientras que $\beta$ es elparámetro `sigma`.
...
Se observa que en este caso la esperanza es la cosntante de uler $\gamma=0.577215664\ldots$.
